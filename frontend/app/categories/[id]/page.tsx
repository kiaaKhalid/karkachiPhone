...
import { ChevronLeft, ChevronRight, X, Grid2X2, Maximize2, SlidersHorizontal } from "lucide-react"
...
export default function CategoryPage() {
  ...
  const normalizeProduct = useCallback((item: ProductItem): Product => {
    ...
    return {
      ...
      brand: brandName ? { name: brandName } : undefined as any,
  ...
  return (
    <div className="container mx-auto px-4 h-screen flex flex-col">
      ...
      <div className="flex-1 flex flex-col lg:flex-row gap-8 min-h-0">
        ...
        <aside className="hidden lg:block w-1/4 p-6 bg-white dark:bg-gray-800 rounded-lg shadow-md flex-shrink-0">
                    ...
                    <Drawer>
                      ...
                      <DrawerContent className="max-h-[85vh] overflow-y-auto [&::-webkit-scrollbar]:hidden">
                        <DrawerHeader>
                          ...
                          <DrawerDescription>Affinez votre recherche par marque ou prix.</DrawerDescription>
                        </DrawerHeader>
              ...
              {viewMode === 'fullscreen' ? (
                <FullscreenCarousel>
                  {products.items.map((product) => (
                    <FullscreenProductCard key={product.id} product={normalizeProduct(product)} />
                  ))}
                </FullscreenCarousel>
              ) : (
                /* Standard Grid View */
                  ...
                  {products.items.map((product) => {
                    ...
                    return (
                      <ProductCard 
                        key={product.id} 
                        product={normProduct} 
                        isInWishlist={isInWishlist(normProduct.id)}
                        onToggleWishlist={() => isInWishlist(normProduct.id) ? removeFromWishlist(normProduct.id) : addToWishlist(normProduct)}
                      />
                    );
                  })}
                </div>
